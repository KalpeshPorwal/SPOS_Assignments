	...
	public void generateCode(String filename) throws Exception
	{
		...
		while((line=br.readLine())!=null)
		{
			...
			if(parts[2].contains("S"))
			{
				...
				code=String.format("%02d", opcode)+"\t"+regcode+"\t"+String.format("%03d", SYMTAB.get(symIndex-1).getAddess())+"\n";
				bw.write(code);
			}
			else if(parts[2].contains("L"))
			{
				int symIndex=Integer.parseInt(parts[2].replaceAll("[^0-9]", ""));
				code=String.format("%02d", opcode)+"\t"+regcode+"\t"+String.format("%03d", LITTAB.get(symIndex-1).getAddess())+"\n";
				bw.write(code);
			}		
			}
